...
def _make_package_labels(pkg, revision, options):
    ...
    package_registry_version = pkg.registry.get_package_version()
    ...
    with _acquire_file_lock(lock_path):
        ...
        for source in sources:
            ...
            with util.tempdir() as tmp_dir:
                ...
                Registry(source, stage_dir).check_package_version(
                    LooseVersion('1.0'),
                    LooseVersion('2.0'))
...
class Registry():
    ...
    def check_package_version(self, min_version, max_version):
        """Checks that the package version is [min_version, max_version)
        ...
        package_version = LooseVersion(self.get_package_version())
        if not (package_version >= min_version and
                package_version < max_version):
            raise DCOSException((
    ...
    def get_package_version(self):
        """Returns the package version of this registry.
...
class Package():
    ...
    def latest_package_revision(self, package_version=None):
        ...
        if package_version:
            ...
            revision = next(
                (pkg_rev for pkg_rev
                    in reversed(pkg_rev_map)
                    if pkg_rev_map[pkg_rev] == package_version), None)
            if not revision:
                raise DCOSException(
                    "No package available with version {}".format(
                        package_version))
        else:
            pkg_revisions = self.package_revisions()

            if not pkg_revisions:
                raise DCOSException(
