...
def setup_module():
    ...
    # start test-app1, test-app2
    with concurrent.futures.ThreadPoolExecutor(10) as pool:
        for app_ in INIT_APPS:
            pool.submit(lambda: add_app(app_[0], True))


def teardown_module():
    with concurrent.futures.ThreadPoolExecutor(10) as pool:
        for app_ in INIT_APPS:
            pool.submit(lambda: remove_app(app_[1]))
...
def test_ls_multiple_tasks():
    stderr = b'Multiple tasks matching ID [test-app]: [test-app], ' + \
             b'[test-app2]'
    returncode, stdout, stderr = exec_command(
    ...
    assert returncode == 1
    assert stdout == b''
    assert stderr.startswith(b'There are multiple tasks with that ID')
